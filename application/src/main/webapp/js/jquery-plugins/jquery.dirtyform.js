      ...
      if ($(this).parents('.ui-tabs-nav').length > 0){
        ...
        tabs.bind('tabsselect.dirty_form', function(event, ui){
          if($('.dirtyform').are_dirty()) {
            ...
            var buttons = {};
            buttons[settings.proceedText] = function(){
            ...
            buttons[settings.cancelText] = function(){$(this).dialog('destroy').remove()};
        ...
        stopper.bind('click.dirty_form', function(event){
          if($('.dirtyform').are_dirty()) {
            ...
            var div = $("<div id='dirty_stopper_dialog'/>").appendTo(document.body),
                href = $(this).attr('href');
            var buttons = {};
            buttons[settings.proceedText] = function(){window.location = href};
            buttons[settings.cancelText] = function(){$(this).dialog('destroy').remove(); return false};
            div.dialog($.extend({
                buttons: buttons
